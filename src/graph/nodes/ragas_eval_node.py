...
from src.rag_service.langfuse import (
    get_langfuse_client,
    langfuse_enabled,
)
...
def _trace_id_from_config(
    config: RunnableConfig | None,
) -> str | None:

    callbacks: Any = (config or {}).get(
        "callbacks",
        [],
    )
...
async def ragas_eval_node(
    state: HealthState,
    config: RunnableConfig | None = None,
) -> dict[str, Any]:

    """
    Evaluate final answer using RAGAS and push metrics to Langfuse.
    ...
    try:
        ...
        if not retrieved_contexts:
            logger.warning(
                "No retrieved contexts found for RAGAS evaluation."
            )
            return {
                "ragas_scores": {}
            }
